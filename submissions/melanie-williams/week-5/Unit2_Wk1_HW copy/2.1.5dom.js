(function () {
  ...
  DOMTree.prototype = {
    ...
    // EXAMPLE:
    // $d('li').each(function(){console.log('this is working')})
    // the argument passed into .each() serves as the callback function that is passed into .each() when we call the property
    // .each() loops through the array and performs the callback function on each element
    ...
    // WHEN WE CALL THE .each() method and pass it the element argument, basically it is saying this.htmlElements[i].innerHTML
...
// WHY DOESN'T THIS CHANGE ALL CLASSES
    ...
    attr: function (key, value) {
      if(!value) {
        ...
        this.each(function(element) {
          var currentAttributes = element.getAttribute(key);
          element.setAttribute(key, value + ' ' + currentAttributes);
        })
      }
    },
