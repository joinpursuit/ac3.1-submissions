(function () {
  ...
  DOMTree.prototype = {
    ...
    each: function (callback) {
      this.htmlElements.forEach(function(val, indx){
        callback.call(val,indx, val)
      })
    },
        ...
        this.each(function(){
          ...
          this.each(function(){
            ...
            Object.keys(event).forEach(function(key){
              that.addEventListener(key, event[key]);
            })
          })
      }
    },
    ...
    children: function(selector) {
      var elements = []//.slice.call(this.htmlElements.childNodes);

      this.each(function(){
        elements = elements.concat([].slice.call(this.childNodes));
      })

      
      return elements.filter(function(x){
        if(selector && x.nodeType === 1 && x.matches(selector)){
          return x;
        } else if(!selector && x.nodeType === 1){
          return x;
        }
      })
